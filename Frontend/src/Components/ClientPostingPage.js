...
function ClientPostingPage() {
  ...
  useEffect(() => {
    ...
    const fetchLocation = async () => {
      const response = await fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${logi}&localityLanguage=en`);
      const data = await response.json();
      setPlace(data.locality);
    }
    fetchLocation();
    fetchCat();
  }, [setCategories, lat, logi]);
